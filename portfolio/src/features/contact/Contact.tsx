...
const SocialIcon = ({ type }: { type: string }) => {
  switch (type) {
    ...
    default:
      return <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="12" r="10"/></svg>;
  }
};
    ...
    <section id="contact" className="relative w-full min-h-screen flex flex-col justify-center py-24 bg-bg-primary">
      <div className="section-wrapper">
        ...
        <div className="mb-16 text-center">
          <p className="text-label mb-3">04 — Commlink</p>
        ...
        <div className="max-w-2xl mx-auto">
          ...
          >
            {/* Name */}
            <div>
              <label className="text-label text-[10px] block mb-2">Your Name</label>
              <input
                type="text"
                value={form.name}
                ...
                required
                className="w-full px-5 py-4 bg-bg-secondary border border-border-subtle text-text-primary text-sm placeholder-text-muted focus:border-accent-teal focus:outline-none transition-colors cut-corner-xs"
              />
            </div>

            {/* Email */}
            <div>
              <label className="text-label text-[10px] block mb-2">Email Address</label>
              <input
                type="email"
                value={form.email}
            ...
            {/* Message */}
            <div>
              <label className="text-label text-[10px] block mb-2">Message</label>
              <textarea
                value={form.message}
                ...
                required
                className="w-full px-5 py-4 bg-bg-secondary border border-border-subtle text-text-primary text-sm placeholder-text-muted focus:border-accent-teal focus:outline-none transition-colors resize-none cut-corner-xs"
              />
            </div>
          ...
          <div className="mt-16 pt-12 border-t border-border-subtle">
            <p className="text-label text-[10px] text-center mb-6">Or find me on</p>
